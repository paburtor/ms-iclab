            ...
            {                                
                script
                {
                    ...
                    if(comment.startsWith('patch:') || comment.startsWith('patch :')) 
                    {
                        ...
                        statuscode=sh(script: "curl -o /dev/null -s -w \"%{http_code}\" -X POST -H \"Accept: application/vnd.github+json\" -H \"Authorization: Bearer $GIT_AUTH\" https://api.github.com/repos/paburtor/ms-iclab/pulls --data-raw '$myjson'", returnStdout: true)                         
