            ...
            {                                
                script
                {
                    ...
                    if(comment.startsWith('patch:') || comment.startsWith('patch :')) 
                    {
                        ...
                        statuscode=sh(script: "curl -X POST -H \"Accept: application/vnd.github+json\" -H \"Authorization: Bearer xxx\" https://api.github.com/repos/paburtor/ms-iclab/pulls -d $myjson", returnStdout: true)                         
